...
function login() {
    if (!attemptingLogin) {
        ...
        sendToWs('login', [['username', $('#username-input').val()], ['password', $('#password-input').val()]])
    }
}
...
function register() {
    if (!attemptingRegister) {
        if ($('#password-register-verify').val() === $('#password-register').val()) {
            ...
            sendToWs('register', [['username', $('#username-register').val()], ['password', $('#password-register').val()]])
        } else {
...
function checkForToken() {
    ...
    if (token !== "") {
        token = token.split('|')
        console.log(token)
        sendToWs("token", [['token', token[0]], ['userId', token[1]]])
    }
}
